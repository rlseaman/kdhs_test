...
main(int argc, char *argv[])
{
    ...
    char msg[64], config[128];
    int i, j, level;
    int mb_fd = 0, port = DEF_PORT, debug = 0;
    ...
    */
    if (use_mbus) {
	if ((mb_tid = mbusConnect("Collector", "client", FALSE)) <= 0) {
	...
	*/ 
	...
	if (console)
	    fprintf (stderr, "Waiting for PAN to connect...\n");
...
{
    printf ("Usage: ");
    printf ("collector -p port -console -temp -[no]mbus -debug 'int >= 10'\n");
}
