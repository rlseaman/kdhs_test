...
void superHandleClient(int socket)
{
   ...
   /* Receives message code */
        ...
        size = dcaRecv (socket, &buffer[0], SZMSG);
        if (size != SZMSG) {
           printf ("superHandleClient: Error receiving message: sz: %d\n",size);
	   return;
        }
...
printf("mtype: %d, DCA_SET: %d, DCA_NOOP: %d\n",mtype,DCA_SET,DCA_NOOP);
    if (mtype & DCA_SET) {
	   ...
	   printf("fpCfg.(x,y)Size: (%d,%d)\n",fpCfg.xSize,fpCfg.ySize);
        ...
        sprintf ((dev= (char *)malloc(SZMSG)), "%s:%d", dhs.dhsNodeName, dhs.dhsCollectorPortNumber);
printf("DCADEV sending Collector name and port: %s, on socket: %d\n",dev,socket);
