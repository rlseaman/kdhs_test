...
char *argv[];
{
	...
	int depth, tile = 0, xflip = 0, yflip = 0;
	...
	for (i=1; i < argc; i++) {
	    ...
	    } else if (strcmp (argv[i], "-xflip") == 0) {
                xflip++;
	    } else if (strcmp (argv[i], "-yflip") == 0) {
                yflip++;
...
fprintf (stderr, "xflip: %d  yflip: %d\n", xflip, yflip);
	xim_setFlip (xim, &xim->frames[0], xflip, yflip);
