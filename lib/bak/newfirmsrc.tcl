...
dprintf 1 "Happy ....\n"
    ...
    global imageid srcdir datafile mapfiles mapunknown maplevel
    global inkwdbs outkwdbs kwdbs dets

dprintf 1 "Happy ....\n"
dprintf 1 "Beginning NEWFIRM translation....\n"
...
dprintf 1 "Saving keywords....\n"
    if {[catch NFSave serr]} {dprintf 1 "$serr\n"}
...
dprintf 1 "Mapping keywords....\n"
    MapKeywords NOCS_Post $mapfiles(NOCS) NOCS yes $maplevel
...
dprintf 1 "Setting detector information....\n"
    SetDets

    # Set database with defaults.
dprintf 1 "Setting default databases....\n"
    DefHeader def

    # Create PHU database.
dprintf 1 "Creating PHU databases....\n"
    PhuHeader phu

    # Create extension headers.
    foreach det $dets {
dprintf 1 "Creating Extensionsion database for $det....\n"
	ExtHeader $det
    }
    ExtClean
    MoveUnknown phu
...
dprintf 1 "Verifying keywords....\n"
    PhuVerify
    ExtVerify

    # Finish up.
dprintf 1 "Finishing....\n"
    foreach kwdb {def NOCS.map Pan.map NOCS PanA PanB} {
...
proc DefHeader name {
    ...
    # Computed values
    SetTel     $name NOCS TELESCOP
    SetDet     $name NOCS
    SetEquin   $name NOCS RAINST DECINST EQUINOX
    SetObstype $name NOCS "NFOBSTYP OBSTYPE" S "Observation type"
    SetFilter  $name NOCS "NFFILTER FILTER"
    SetDate    $name NOCS UTDATE UT DATE-OBS S "Date of observation start (UTC)"
...
AddEntry phu NAXIS 0 N
    ...
    MoveEntry phu ""   "TITLE OBJECT"   "Observation title"
    ...
    MoveEntry phu NOCS "RA OBJRA"
    MoveEntry phu NOCS "DEC OBJRA"
    MoveEntry phu NOCS "EPOCH OBJEPOCH"
    ...
    MoveEntry phu NOCS  "TCPHA HA"
    MoveEntry phu NOCS  ZD        "Zenith distance"
    MoveEntry phu NOCS  AIRMASS   "Airmass" "" N
...
#    if {![catch {GetValue "" GTINDEX}]} {CopyEntry phu "" "GTINDEX ARCONGI"}
    ...
    AddEntry  phu      ""
    MoveEntry phu ""   "ASTRONOM OBSERVER" "Observer(s)"
#    MoveEntry phu ""   PROPOSER "Proposer(s)"
#    MoveEntry phu ""   PROPOSAL "Proposal title"
    MoveEntry phu ""   "PROPOSAL PROPID"   "Proposal identification"
    MoveEntry phu ""   OBSID
...
#				"Keyword dictionary"
    ...
    AddEntry  phu      RECNO              0 N "NOAO archive sequence number"
}
...
proc NFSave {} {
    global imageid fname kwdbs

    if {![info exists fname]} return

    dprintf 1 $imageid "Save keywords to $fname\n"
    if {[catch {set f [open $fname w]} err]} {
...
proc MoveUnknown {outkwdb} {
    global kwdbs outkwdbs imageid mapfiles
...
proc FindEntry {kw} {
    ...
    lappend search NOCS PanA PanB phu def
    foreach kwdb $search {
...
proc SetEquin {outkwdb inkwdb rakw deckw eqkw} {
    ...
    if {[catch {
	# Get the RA/DEC.
	MoveEntry $outkwdb $inkwdb "$rakw  TELRA"    "RA of telescope (hr)"
	MoveEntry $outkwdb $inkwdb "$deckw TELDEC"   "DEC of telescope (deg)"
...
proc SetTel {outkwdb inkwdb kws args} {
	...
	kp4m {
	    ...
	    MoveEntry $outkwdb $inkwdb "CASSADC ADCSTAT"
            ...
            MoveEntry $outkwdb $inkwdb  "FOCUS TELFOCUS"
	    }
	}
    } err]} {
	dprintf 1 $imageid \
...
proc SetDate {outkwdb inkwdb datekw timekw kw type comment} {
    ...
    if {[catch {
	set date [GetValue $inkwdb $datekw]
        set time [GetValue $inkwdb $timekw]]
	DeleteEntry $inkwdb $datekw
...
proc SetWcs {ext} {
    ...
    global WAT01 WAT11 WAT12 WAT13 WAT14 WAT15 WAT16
    global WAT21 WAT22 WAT23 WAT24 WAT25 WAT26
...
proc SetDets {} {
    ...
    global ndets dets
    global extname mosname mossize detname detsize
    global detsec datasec trimsec biassec
    global bpm wcsfilter
    global LTV1 LTV2
    global WCSASTRM CTYPE1 CTYPE2
    global CRVAL1 CRVAL2 CRPIX1 CRPIX2 CD1_1 CD2_2 CD1_2 CD2_1
    global WAT01 WAT11 WAT12 WAT13 WAT14 WAT15
    global WAT21 WAT22 WAT23 WAT24 WAT25
