...
proc kosmos {} {
    ...
    if {[catch { KosSave } err]} {dprintf 1 "KosSave Error: $err\n"}
...
proc DefHeader name {
    ...
    AddEntry $name INSTRUME "KOSMOS" S "Science Instrument"
...
proc PhuHeader name {
    ...
    # The following keywords were set in "def" in the SetTel procedure.
    ...
    MoveEntry phu ""  "MSEFILTR FILPOS" "Filter position"
    MoveEntry phu ""  "NOCFSN FILTERSN" "Filter Serial Number"
...
proc ExtHeader {det} {
    ...
    global extname detsize detname detval
    ...
    if {[info exists detval(gain,$det)]} {
        AddEntry $ext GAIN [expr int(10./$detval(gain,$det))/10.] N "Inv gain (ADU/e)"
    } elseif {[info exists detval(gain,approx)]} {
        AddEntry $ext GAIN [expr int(10./$detval(gain,approx))/10.] N "Approx inv gain (ADU/e)"
    }
...
# KosSave -- Save all NOCS and Pan keywords in a compact format.
...
proc KosSave {} {
    ...
    if {[info exists kwdbs(NOCS_Post)]} {
        ...
        dprintf 1 $imageid "KosSave: No NOCS keywords available, skipping....\n"
    }
    ...
    if {[info exists kwdbs(PanA_Post)] } {
        ...
        dprintf 1 $imageid "KosSave: No PAN keywords available, skipping....\n"
    }
...
proc MapNOCS {inkwdb mapkwdb outroot mapunknown maplevel} {
	...
	AddEntry $outkwdb NOCINST "KOSMOS"           S "Science Instrument"
    }
...
proc SetDet {outkwdb inkwdb} {
	...
	AddEntry $outkwdb INSTRUME $detname(mos) S "Kosmos detector"
	AddEntry $outkwdb DETSIZE  $size S "Kosmos detector size"
	AddEntry $outkwdb NDETS  $ndets N "Number of detectors in kosmos"
...
proc SetTel {outkwdb inkwdb kws args} {
	...
	kp4m {
		...
		"Mayall Corrector" S "Corrector Identification"
            ...
            MoveEntry $outkwdb $inkwdb  "NOCFOCUS TELFOCUS"
	    }
	kp09m {
...
proc CcdGeom {outkwdb ccd amp ext} {
	...
	set ltm(1) [expr double($ltm(1))/$colbin]
	set ltv(1) [expr 1.-$ltm(1)*$cx1-(1.-$ltm(1))/2.]
	set ltm(2) [expr double($ltm(2))/$rowbin]
	set ltv(2) [expr 1.-$ltm(2)*$cy1-(1.-$ltm(2))/2.]

	# Set the sections.
	set ccdsec [format "\[%d:%d,%d:%d\]" $cx1 $cx2 $cy1 $cy2]

	set ic1 [expr int($ltm(1)*$cx1+$ltv(1)+0.5)]
	set ic2 [expr int($ltm(1)*$cx2+$ltv(1)+0.5)]
	set il1 [expr int($ltm(2)*$cy1+$ltv(2)+0.5)]
	set il2 [expr int($ltm(2)*$cy2+$ltv(2)+0.5)]
	...
	AddEntry $outkwdb LTM1_1 $ltm(1)      N "CCD to image transformation"
	AddEntry $outkwdb LTM2_2 $ltm(2)      N "CCD to image transformation"
	AddEntry $outkwdb LTV1   $ltv(1)      N "CCD to image transformation"
	AddEntry $outkwdb LTV2   $ltv(2)      N "CCD to image transformation"
