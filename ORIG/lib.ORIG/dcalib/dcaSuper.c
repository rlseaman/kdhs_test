...
dcaSupOpenConn (char *host, int port, char *name)
{
    ...
    memset (domain, 0, SZMSG);		/* get the local host name	*/
    (void) getdomainname (domain, SZMSG);
    if (strncmp (domain, "(none)", 6) == 0)
	strcpy (domain, "kpno.noao.edu");

    memset (hostname, 0, SZMSG);	/* get the local host name	*/
...
fprintf (stderr, "\tuse_sim: %d  act='%s'  sim='%s'  dom='%s'\n\n",
    ...
    /* Reconnect request.
     */
    memset (dev, 0, SZMSG); 		/* tell em we're ready  */
    strcpy (dev, &msg[14]);
    ...
    memset (msg, 0, SZMSG); 		/* tell em we're ready  */
...
dcaSupParseNode (char *host, int port, unsigned short *host_port, unsigned long *host_addr)
{
    ...
    memset (lhost, 0, SZMSG);
    memset (host_str, 0, SZMSG);
    memset (port_str, 0, SZMSG);
